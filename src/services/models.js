...
// 將 video 目前畫面前處理進 640×640 正方形 canvas（9:16 裁切 + letterbox，無旋轉修正）
// 回傳換算所需的幾何參數，供座標還原。
function preprocessForDetection(video, canvas) {
...
export function runCarDetection(video, model, canvas) {
  const { cropW, cropH, scale, padLeft, padTop } = preprocessForDetection(video, canvas);
